...
if __name__ == '__main__':
    ...
    dataset_loader = torch.utils.data.DataLoader(dataset, batch_size=opt.batch_size, shuffle=True, num_workers=16, drop_last = True)
    ...
    if opt.stage == 'full' or opt.stage == 'instance':
        for epoch in trange(opt.epoch_count, opt.niter + opt.niter_decay, desc='epoch', dynamic_ncols=True):
            ...
            for data_raw in tqdm(dataset_loader, desc='batch', dynamic_ncols=True, leave=False):
                ...
                data_raw['gt_img'] = [data_raw['gt_img']]
                data_raw['input_img'] = [data_raw['input_img']]


                input_data = util.get_colorization_data(data_raw['input_img'], opt, p=1.0, ab_thresh=0)
                gt_data = util.get_colorization_data(data_raw['gt_img'], opt, p=1.0, ab_thresh=10.0)
                if gt_data is None:
                    continue
        ...
        for epoch in trange(opt.epoch_count, opt.niter + opt.niter_decay, desc='epoch', dynamic_ncols=True):
            ...
            for data_raw in tqdm(dataset_loader, desc='batch', dynamic_ncols=True, leave=False):
                ...
                cropped_input_data = util.get_colorization_data(data_raw['cropped_input'], opt, p=1.0, ab_thresh=0)
                cropped_gt_data = util.get_colorization_data(data_raw['cropped_gt'], opt, p=1.0, ab_thresh=10.0)
                full_input_data = util.get_colorization_data(data_raw['full_input'], opt, p=1.0, ab_thresh=0)
                full_gt_data = util.get_colorization_data(data_raw['full_gt'], opt, p=1.0, ab_thresh=10.0)
