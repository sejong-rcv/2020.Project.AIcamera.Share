...
parser.add_argument('--dataset', type=str, default='KAIST_DATASET')
...
for img_name in tqdm(image_list):     
    img = cv2.imread(join(input_dir, img_name))
    lab_image = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
    l_channel, a_channel, b_channel = cv2.split(lab_image)
    l_stack = np.stack([l_channel, l_channel, l_channel], axis=2)
    outputs = predictor(l_stack)
    save_path = join(output_npz_dir, img_name.split('.')[0])
    ...
    if args.filter_no_obj is True and pred_bbox.shape[0] == 0:
        print('delete {0}'.format(img_name))
        
        ##potenit
        if args.dataset=='KAIST_DATASET':
            name1 = img_name.split('LEFT')[1]
            os.remove(join(input_dir, img_name))
            os.remove(join(ther_dir, 'THER'+name1))
            continue
        else:
            ##KAIST_DATASET
            os.remove(join(input_dir, img_name))
            os.remove(join(ther_dir, img_name))
            continue
